...
export default function SuperAdminHotelRoomForm({ hotel, room = null }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        if (isEditing) {
            put(route('superadmin.hotels.rooms.update', [hotel.id, room.id]));
        } else {
            post(route('superadmin.hotels.rooms.store', hotel.id));
        }
    };

    return (
        <AuthenticatedLayout
            header={isEditing ? `Editar Quarto (${hotel.name})` : `Novo Quarto (${hotel.name})`}
        >
            <div className="py-12">
